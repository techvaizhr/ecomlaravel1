...
@section('title', 'Pending Customer Reviews')
...
<div class="container-fluid py-3">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <div>
            ...
            <p class="text-muted small mb-0">Review and approve customer submissions before they appear on the store.</p>
        </div>
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('reviews.index') }}" class="btn btn-outline-primary px-3 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-4">
            <div class="stat-card-gradient" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);">
                ...
                <div class="small text-white-50 text-uppercase fw-bold">Pending Reviews</div>
    ...
    <div class="card card-modern mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('reviews.pending') }}" id="filterForm">
                <div class="row g-2 align-items-center">
                    {{-- Keyword Search --}}
                    <div class="col-12 col-md-4">
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-white border-end-0"><i data-feather="search" style="width: 14px;"></i></span>
                            <input type="text" name="keyword" value="{{ request('keyword') }}" class="form-control border-start-0" placeholder="Search customer, product, comment...">
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-3 d-flex gap-2 justify-content-end align-items-center">
                        ...
                        @if(request()->anyFilled(['keyword', 'ratting', 'start_date', 'end_date', 'date_preset']))
                            <a href="{{ route('reviews.pending') }}" class="btn btn-outline-secondary btn-sm" title="Reset Filters"><i data-feather="rotate-ccw" style="width: 14px;"></i></a>
                        @endif
                    </div>
                </div>
            </form>
                ...
                <tbody>
                    @forelse($data as $key => $value)
                        <tr>
                            ...
                            {{-- Comment --}}
                            <td>
                                <p class="mb-0 text-dark small text-break" style="line-height: 1.4;">
                                    {{ $value->review }}
                                </p>
                            </td>
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-1">
                                    <form action="{{ route('reviews.active') }}" method="POST" class="d-inline">
                                        ...
                                        <button type="submit" class="btn btn-sm btn-success px-2 py-1 d-inline-flex align-items-center gap-1" title="Approve & Publish">
                                    ...
                                    <form action="{{ route('reviews.destroy') }}" method="POST" class="d-inline"
                                          onsubmit="return confirm('Are you sure you want to delete this pending review?');">
                                        @csrf
                                        <input type="hidden" name="hidden_id" value="{{ $value->id }}">
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                <div class="text-muted">
                                    <i data-feather="check-circle" style="width: 48px; height: 48px; opacity: 0.35;" class="mb-2 text-success"></i>
                                    <p class="fw-bold mb-1">No Pending Reviews</p>
                                    <small class="text-muted">All customer reviews are up to date and reviewed!</small>
                                </div>
                            </td>
                        </tr>
                    @endforelse
