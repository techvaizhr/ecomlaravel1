...
@section('title', 'Manage Google Tag Manager')
...
<div class="container-fluid py-3">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-2">
        <div>
            <h4 class="mb-1 fw-bold text-dark d-flex align-items-center gap-2">
                <i data-feather="tag" class="text-info" style="width: 24px; height: 24px;"></i>
                Google Tag Manager (GTM)
            </h4>
            <p class="text-muted small mb-0">Manage Google Tag Manager container IDs for comprehensive event and analytics tracking.</p>
        </div>
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('tagmanagers.create') }}" class="btn btn-primary px-3 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2">
                <i data-feather="plus-circle" style="width: 16px; height: 16px;"></i>
                <span>Add GTM Container</span>
            </a>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-12 col-md-4">
            <div class="stat-card-gradient" style="background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%);">
                <div class="stat-icon"><i class="fas fa-tags"></i></div>
                <div class="small text-white-50 text-uppercase fw-bold">Total Containers</div>
    ...
    <div class="card card-modern mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('tagmanagers.index') }}" id="filterForm">
                <div class="row g-2 align-items-center">
                    {{-- Keyword Search --}}
                    <div class="col-12 col-md-4">
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-white border-end-0"><i data-feather="search" style="width: 14px;"></i></span>
                            <input type="text" name="keyword" value="{{ request('keyword') }}" class="form-control border-start-0" placeholder="Search GTM ID (e.g. GTM-XXXX)...">
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-3 d-flex gap-2 justify-content-end align-items-center">
                        ...
                        @if(request()->anyFilled(['keyword', 'status', 'start_date', 'end_date', 'date_preset']))
                            <a href="{{ route('tagmanagers.index') }}" class="btn btn-outline-secondary btn-sm" title="Reset Filters"><i data-feather="rotate-ccw" style="width: 14px;"></i></a>
                        @endif
                    </div>
                </div>
            </form>
                ...
                <tbody>
                    @forelse($data as $key => $value)
                        <tr>
                            ...
                            {{-- Code --}}
                            <td>
                                <span class="gtm-badge" onclick="copyGtmCode('{{ $value->code }}')" title="Click to copy">
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-1">
                                    @if($value->status == 1)
                                        <form action="{{ route('tagmanagers.inactive') }}" method="POST" class="d-inline">
                                            @csrf
                                            <input type="hidden" name="hidden_id" value="{{ $value->id }}">
                                            <button type="submit" class="btn-action-icon text-warning" title="Deactivate Container">
                                                <i data-feather="pause-circle" style="width:14px; height:14px;"></i>
                                            </button>
                                        </form>
                                    @else
                                        <form action="{{ route('tagmanagers.active') }}" method="POST" class="d-inline">
                                            @csrf
                                            <input type="hidden" name="hidden_id" value="{{ $value->id }}">
                                            <button type="submit" class="btn-action-icon text-success" title="Activate Container">
                                                <i data-feather="check-circle" style="width:14px; height:14px;"></i>
                                            </button>
                                        </form>
                                    @endif

                                    <a href="{{ route('tagmanagers.edit', $value->id) }}" class="btn-action-icon text-primary" title="Edit GTM">
                                        <i data-feather="edit-2" style="width:14px; height:14px;"></i>
                                    </a>
                                    
                                    <form action="{{ route('tagmanagers.destroy') }}" method="POST" class="d-inline"
                                          onsubmit="return confirm('Are you sure you want to delete this GTM container?');">
                                        @csrf
                                        <input type="hidden" name="hidden_id" value="{{ $value->id }}">
                                        <button type="submit" class="btn-action-icon text-danger" title="Delete GTM">
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5">
                                <div class="text-muted">
                                    <i data-feather="tag" style="width: 48px; height: 48px; opacity: 0.35;" class="mb-2"></i>
                                    <p class="fw-bold mb-1">No GTM Containers Found</p>
                                    <small class="text-muted">Add your Google Tag Manager Container ID (GTM-XXXXXX) for custom scripts.</small>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
<script>
    function copyGtmCode(code) {
        navigator.clipboard.writeText(code).then(() => {
            toastr.success('GTM Code "' + code + '" copied to clipboard!');
        });
    }
</script>
@endpush