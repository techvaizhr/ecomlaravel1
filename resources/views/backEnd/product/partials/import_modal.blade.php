    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 18px; overflow: hidden;">
            ...
            <div class="modal-body p-4" style="background: #f8fafc;">
                ...
                {{-- STEP 1: URL INPUT CARD (CLEAN & DIRECT) --}}
                                ...
                                <div class="card border-0 shadow-sm mb-3" style="border-radius: 12px;">
                                    <div class="card-body p-3">
                                        ...
                                        <div class="d-flex align-items-center gap-1 p-1.5 mb-2 rounded bg-light border text-muted" style="font-size: 11px;">
                                            <i class="fe-move text-primary"></i>
                                            <span>ছবি ড্র্যাগ করে সাজান অথবা <strong>"⭐ Set Main"</strong> বাটনে ক্লিক করে প্রধান ছবি নির্ধারণ করুন।</span>
                                        </div>

                                        <div id="imp_images_grid" class="d-flex flex-wrap gap-2" style="max-height: 250px; overflow-y: auto; padding: 4px;">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleCardDrop(e) {
        ...
        if (draggedCard && this !== draggedCard) {
            const allCards = Array.from(imagesGrid.querySelectorAll('.imp-img-card'));
    ...
    function handleCardDragEnd() {
        ...
        document.querySelectorAll('#imp_images_grid .imp-img-card').forEach(card => {
    ...
    function updateImpImageBadges() {
        const cards = imagesGrid.querySelectorAll('.imp-img-card');
        cards.forEach((card, idx) => {
            ...
            if (idx === 0) {
                ...
                footer.innerHTML = `<span class="badge bg-primary w-100 py-1" style="font-size:9px; border-radius:3px;"><i class="fe-star me-0.5"></i> Main</span>`;
            } else {
                card.style.borderColor = '#e2e8f0';
                card.style.boxShadow = 'none';
                footer.innerHTML = `<button type="button" class="btn btn-xs btn-light w-100 py-0 border shadow-sm btn-make-imp-main" style="font-size:8.5px; font-weight:600; color:#1e293b; border-radius:3px;" title="প্রধান ছবি নির্বাচন করুন"><i class="fe-star text-warning me-0.5"></i> Set Main</button>`;
            }
        });
    }

    // Set Main Image Click Handler
    $(document).on('click', '.btn-make-imp-main', function (e) {
        e.preventDefault();
        e.stopPropagation();
        const card = $(this).closest('.imp-img-card')[0];
        if (card && imagesGrid) {
        ...
        if (images.length === 0) {
            ...
            images.forEach((imgUrl) => {
                ...
                col.innerHTML = `
                    ...
                    <span class="position-absolute bg-dark bg-opacity-75 text-white rounded-circle d-flex align-items-center justify-content-center" style="top:4px; left:4px; width:18px; height:18px; font-size:10px; cursor:grab;" title="Drag to reorder"><i class="fe-move"></i></span>
    ...
    document.getElementById('btn_open_full_edit').addEventListener('click', function () {
        ...
        // Gather selected images in current dragged DOM order
        const selectedImgs = [];
        document.querySelectorAll('#imp_images_grid .imp-img-card').forEach(card => {
