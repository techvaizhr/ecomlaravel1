        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <h4 class="text-white mb-1 fw-bold">
                    <i class="fe-dollar-sign me-2"></i> Vendor Withdrawal Payouts
                </h4>
                <p class="mb-0 text-white-50 font-size-13">
                    Review and process payout requests to vendor bank accounts and mobile wallets.
                </p>
            </div>
            ...
            <div class="row g-2 align-items-end">
                <div class="col-md-3">
                    ...
                    <input type="text" name="keyword" class="form-control form-control-modern" 
                           placeholder="Shop name, account no, beneficiary..." value="{{ request('keyword') }}">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <div class="d-flex gap-1">
                        ...
                        @if(request()->anyFilled(['keyword', 'status', 'payout_method', 'date_from', 'per_page']))
                            <a href="{{ route('admin.vendor.withdrawals.index') }}" class="btn btn-light border rounded-3 px-3 py-2" title="Reset Filters">
            ...
            <table class="table table-modern align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="fw-bold text-dark font-size-14">{{ $row->vendor->shop_name ?? 'Vendor #' . $row->vendor_id }}</div>
                            <span class="text-muted font-size-12">Owner: {{ $row->vendor->owner_name ?? 'N/A' }}</span>
                        </td>
                        <td>
                        ...
                        <td>
                            @php
                                $methodLower = strtolower($row->payout_method);
                        ...
                        <td class="text-end">
                            @if($row->status === 'pending')
                                <div class="d-inline-flex gap-1">
                                    <button type="button" class="action-circle-btn btn-action-approve" title="Approve Payout" data-bs-toggle="modal" data-bs-target="#modalApproveWithdraw{{ $row->id }}">
                                        <i class="fe-check"></i>
                                    </button>
                                    <button type="button" class="action-circle-btn btn-action-reject" title="Reject Payout" data-bs-toggle="modal" data-bs-target="#modalRejectWithdraw{{ $row->id }}">
                                        <i class="fe-x"></i>
                                    </button>
                                </div>

                                {{-- Approve Modal --}}
                                <div class="modal fade text-start" id="modalApproveWithdraw{{ $row->id }}" tabindex="-1" aria-hidden="true">
                                                ...
                                                <div class="modal-body p-4">
                                                    <div class="alert alert-light border mb-3">
                                                        <div class="d-flex justify-content-between mb-1">
                                                            ...
                                                            <strong class="text-dark">{{ $row->vendor->shop_name ?? 'Vendor' }}</strong>
                                                        </div>
                                                        <div class="d-flex justify-content-between mb-1">
                                ...
                                {{-- Reject Modal --}}
                                <div class="modal fade text-start" id="modalRejectWithdraw{{ $row->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content border-0 shadow">
                                            <form action="{{ route('admin.vendor.withdrawals.reject', $row->id) }}" method="POST">
                                                ...
                                                <div class="modal-body p-4">
                                                    <p class="text-muted font-size-13 mb-3">
                                                        The requested amount of <strong>৳{{ number_format($row->amount, 2) }}</strong> will be instantly refunded back to the vendor's wallet balance.
                                                    </p>
                                                    <div class="mb-3">
