...
@section('title', 'Delivery Districts - ' . $division->name)
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div>
            <a href="{{ route('admin.delivery.divisions.index') }}" class="btn btn-sm btn-outline-secondary rounded-pill mb-2 px-3">
                <i data-feather="arrow-left" class="me-1" style="width:14px;height:14px;"></i> Back to Divisions
            </a>
            <h4 class="mb-1 fw-bold text-dark d-flex align-items-center">
                <i data-feather="map" class="text-primary me-2" style="width:24px;height:24px;"></i>
                Districts in {{ $division->name }} Division
            </h4>
            <p class="text-muted small mb-0">Set default delivery charges and manage sub-upazilas.</p>
        </div>
        <a href="{{ route('admin.delivery.districts.create', ['division' => $division->id]) }}" class="btn btn-primary rounded-pill px-4 shadow-sm">
            <i data-feather="plus" class="me-1" style="width:16px;height:16px;"></i> Add District
        </a>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="metric-card">
                ...
                <div>
                    <div class="text-muted small fw-semibold text-uppercase">Total Districts</div>
        ...
        <div class="col-md-4">
            <div class="metric-card">
                ...
                <div>
                    <div class="text-muted small fw-semibold text-uppercase">Active Districts</div>
        ...
        <div class="col-md-4">
            <div class="metric-card">
                ...
                <div>
                    <div class="text-muted small fw-semibold text-uppercase">Inactive Districts</div>
    ...
    <div class="card card-modern mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('admin.delivery.districts.index', $division->id) }}">
                <div class="row g-2 align-items-center">
                    <div class="col-md-5">
                        <div class="input-group">
                            <span class="input-group-text bg-white border-end-0 text-muted"><i data-feather="search" style="width:16px;"></i></span>
                            <input type="text" name="keyword" class="form-control border-start-0 ps-0" placeholder="Search district name..." value="{{ request('keyword') }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2 d-flex gap-2">
                        ...
                        @if(request()->anyFilled(['keyword', 'status', 'per_page']))
                            <a href="{{ route('admin.delivery.districts.index', $division->id) }}" class="btn btn-outline-secondary" title="Reset Filters">
                    ...
                    <tbody>
                        @forelse($show_data as $key => $value)
                            <tr>
                                ...
                                <td><span class="badge bg-light text-primary border px-2 py-1 fw-bold fs-6">৳{{ number_format($value->delivery_charge, 2) }}</span></td>
                                ...
                                <td class="text-end">
                                    <div class="d-inline-flex gap-1 justify-content-end">
                                        <a href="{{ route('admin.delivery.upazilas.index', $value->id) }}" class="btn btn-sm btn-outline-info rounded-pill px-3">
                                            <i data-feather="map-pin" class="me-1" style="width:14px;height:14px;"></i> Upazilas
                                        </a>
                                        <a href="{{ route('admin.delivery.districts.edit', $value->id) }}" class="btn btn-sm btn-primary rounded-pill px-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    ...
                                    <p class="mb-0">No districts found for this division.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if($show_data->hasPages())
                <div class="card-footer bg-white border-top py-3 d-flex justify-content-between align-items-center">
                    <div class="text-muted small">
                        Showing {{ $show_data->firstItem() }} to {{ $show_data->lastItem() }} of {{ $show_data->total() }} districts
                    </div>
                    <div>
                        {{ $show_data->links() }}
                    </div>
...
document.querySelectorAll('.delete-form-delivery').forEach(function(form) {
    form.addEventListener('submit', function(e) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'This will delete the district and its upazilas!',
            icon: 'warning',
